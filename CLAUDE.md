...
- **Audience-as-BFF + Service-style BC API (ADR-011)**: bounded contexts expose a **service** (`Application/Service/*Service.php`) with typed method calls. They do **not** register handlers on the CQRS bus and do **not** appear in any `SecurityConfig`. Audiences (`Audience/*/Application/Query/*`) own the CQRS messages; HTTP-input parsing is delegated to per-action Form classes (ADR-015). Audiences dispatch through the bus once per page and delegate to BC services by direct method call. Audience handlers always return an **audience-owned** `*Response` (anti-corruption layer), even when it wraps a single BC read model 1:1. Composite queries are the canonical answer to "one page needs multiple BC reads" — one dispatch, one decorator chain pass, one atomic decision.
...
- **Audience query** = `Audience/<Audience>/Application/Query/<UseCase>/{Query,Handler,Response}.php`. Query constructor takes typed VOs/primitives; raw HTTP parsing lives in a per-action Form class (ADR-015), not on the Query. Register in `<Audience>QueryHandlerRegistry` and `<Audience>SecurityConfig` (`null` = public per ADR-008). Handler calls BC services as methods, not through the bus. ADR-011.
...
- **Storage public/private convention (ADR-003)**: paths under `public/<area>/` (e.g. `public/images/`) are world-readable through a per-area CDN subdomain whose origin path is that prefix; anything else (`users/123/...`, internal CSVs) stays private. The subdomain hides the prefix from the URL (`public/images/logo.png` → `https://images.php-ddd.test/logo.png`). New area = new subdomain + new `CdnUrlResolver` mapping entry; never widen an existing one to cover `public/` as a whole.
...
- **Marker interfaces for per-audience seams (ADR-014)**: when a service has the same contract across admin/partner/site but DI must inject the right one, define a base interface and three empty marker subinterfaces (`SessionAuthenticatorInterface` ← `AdminSessionAuthenticatorInterface`, etc.). Bind one-to-one in `di.php` via `DI\autowire`, no named string keys. Common logic lives in an abstract parent class; concrete subclasses redeclare the constructor with audience-typed marker interfaces so PHP-DI can disambiguate the per-audience repository / verifier / etc. Reference: `SharedKernel\Domain\Security\PasswordVerifier\*`, `SharedKernel\Infrastructure\Security\LocalPasswordVerifier` + 3 concretes.
- **Interface families live in feature subfolders, not flat under `Security/`** (or any Domain dir with multiple families). One folder per concept holds the base interface plus its markers: `Security/PasswordVerifier/`, `Security/SessionAuthenticator/`, `Security/RememberMe/`, `Security/UserRepository/`. The cross-family value objects and shared interfaces stay at the root of the parent dir.
- **Audience-scoped command/handler layout**: `Audience/<Audience>/Application/Command/<Feature>/<Verb>Command.php` + `<Verb>Handler.php` (suffix-based, no nested `LogIn/`/`LogOut/` directories). Class names carry the verb (`LogInCommand`, `LogOutHandler`); namespace already discriminates audience. Symmetric with the query layout (`Application/Query/<UseCase>/<UseCase>Query.php`). Reference: `Audience/Admin/Application/Command/Auth/`.
- **Per-audience redirects**: every audience controller stack exposes `protected function redirect(string $path = ''): Response` in its `Controller_<Audience>_Abstract`. Leaf controllers call `$this->redirect()` / `$this->redirect('login')` and never construct absolute URLs by hand. Base URLs live in `app/config/audience.php` (autoloaded via `always_load.config`), seeded from `APP_URL` / `APP_URL_ADMIN` / `APP_URL_PARTNER` env vars. This is the only correct way to build audience URLs — `Response::redirect('/')` resolves through global `base_url` and lands on the main host, not the current subdomain.
- **Repository field naming**: name the field/parameter after **what** is stored, not the technical role — `private OrderRepositoryInterface $orders`, `private UserRepositoryInterface $users`, `private RememberTokenRepositoryInterface $tokens`. The type already says "repository"; `$repository` / `$xxxRepository` are noise. Aggregate roots use plural (`$users`, `$products`); single-aggregate stores use the entity word (`$cart`).
