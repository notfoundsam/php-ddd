...
class Form_Site_Login
{
    ...
    public static function fromHttpInput(array $raw): self
    {
        ...
        $v = Validation::forge('site_login');
        $v->add_field('email', __('site/login.email'), 'required|valid_email');
        $v->add_field('password', __('site/login.password'), 'required|max_length[72]');
