...
class Form_Partner_Login
{
    ...
    public static function fromHttpInput(array $raw): self
    {
        ...
        $v = Validation::forge('partner_login');
        $v->add_field('email', __('partner/login.email'), 'required|valid_email');
        $v->add_field('password', __('partner/login.password'), 'required|max_length[72]');
