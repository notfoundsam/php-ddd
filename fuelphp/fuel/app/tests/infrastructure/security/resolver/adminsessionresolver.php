...
{
    public function test_returns_user_and_calls_findById_when_session_type_matches_audience()
    {
        $admin = new AuthenticatedUser('42', 'admin@example.com', ['admin'], UserType::ADMIN);
        $users = new AdminSessionResolverTestSpyUsers($admin);
        ...
        $this->assertSame(['42'], $users->findByIdCalls);
    }

    public function test_returns_null_and_skips_findById_when_session_type_is_for_another_audience()
    {
        ...
        $this->assertSame([], $users->findByIdCalls, 'foreign user_type must short-circuit before the repository call');
    }

    public function test_returns_null_and_skips_findById_when_session_type_is_missing()
    {
        $admin = new AuthenticatedUser('42', 'admin@example.com', ['admin'], UserType::ADMIN);
        $users = new AdminSessionResolverTestSpyUsers($admin);
...
final class AdminSessionResolverTestSpyUsers implements AdminUserRepositoryInterface
{
